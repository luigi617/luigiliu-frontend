...
const TypingInput: React.FC<TypingInputProps> = ({
  ...
  useEffect(() => {
    setInputHistory(
      words.map(
        (word, wordIndex) => (
          get_word_with_space(word, wordIndex).split('').map(
            (char) => ({ char, isCorrect: null })
          )
        )
      )
    );
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const accuracy = ((numberCurrentChar - wrongWordCount) / numberCurrentChar) * 100;
      showStatistics(grossWpm, netWpm, accuracy);
    },
    [numberCurrentChar, currentCharIndex, currentWordIndex, inputHistory, wrongWordCount, startTime, words]
  );
    ...
    >
      {inputHistory.map((word, wordIndex) => (
        ...
        >
          ...
          {/* {wordIndex !== words.length - 1 ? <span key={-1}>&nbsp;</span> : null} */}
        </span>
      ))}
    </div>
  );
